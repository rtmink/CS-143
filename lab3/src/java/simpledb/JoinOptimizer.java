...
public class JoinOptimizer {
    ...
    public double estimateJoinCost(LogicalJoinNode j, int card1, int card2,
        ...
        if (j instanceof LogicalSubplanJoinNode) {
        	...
        	return cost1 + (card1 * cost2) // I/O cost
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
    	...
    	if (joinOp == Predicate.Op.GREATER_THAN || joinOp == Predicate.Op.GREATER_THAN_OR_EQ 
            ...
            TableStats ts1 = stats.get(table1Name);
            TableStats ts2 = stats.get(table2Name);
            
            TupleDesc td1 = Database.getCatalog().getTupleDesc(table1ID);
            TupleDesc td2 = Database.getCatalog().getTupleDesc(table2ID);
            
            double s1 = ts1.avgSelectivity(td1.fieldNameToIndex(field1PureName), joinOp);
            double s2 = ts2.avgSelectivity(td2.fieldNameToIndex(field2PureName), joinOp);
            
            card = (int)(s1 * card1 * s2 * card2);
    	}
    	
    	// Equality Joins
    	else if (joinOp == Predicate.Op.EQUALS || joinOp == Predicate.Op.LIKE) {
    		
    		if (t1pkey && t2pkey) // Both tables have primary keys
    ...
    public Vector<LogicalJoinNode> orderJoins(
    	...
    	// Selinger's Algorithm according to the spec
    	PlanCache planCache = new PlanCache();
    	
    	for (int i = 0; i < joins.size(); i++) {
    		
    		for (Set<LogicalJoinNode> s : enumerateSubsets(joins, i+1)) { // IMPORTANT: add 1 to i
    			...
    			for (LogicalJoinNode node : s) {
    				...
    				CostCard subplan = computeCostAndCardOfSubplan(stats, filterSelectivities, node, s, bestPlan.cost, planCache);
    			...
    			// Add the best plan so far (if any) to cache
    			if (bestPlan.cost != Double.MAX_VALUE)
    				planCache.addPlan(s, bestPlan.cost, bestPlan.card, bestPlan.plan);
    		}
    	}
    	
    	Vector<LogicalJoinNode> joinPlan = planCache.getOrder(new HashSet<LogicalJoinNode>(joins));
    	if (joinPlan == null)
    		joinPlan = joins;
    	
    	if (explain)
    		printJoins(joinPlan, planCache, stats, filterSelectivities);
